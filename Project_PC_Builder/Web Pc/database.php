<?php
/**
 * DATABASE KOMPONEN PC
 * Generator data komponen untuk PC Builder AI
 * Konversi dari database.py ke PHP Native
 */
...
function generate_data() {
    global $cpu_templates, $gpu_templates, $mobo_templates, $ram_templates;
    global $psu_templates, $storage_templates, $case_templates;
    global $brands_gpu, $brands_mobo, $brands_ram, $brands_psu, $brands_case, $brands_storage;
    ...
    // Generate GPU
    foreach ($gpu_templates as $tmpl) {
        foreach ($brands_gpu as $brand) {
            // Variasi harga acak +- 10% biar realistis
            $price_variation = $tmpl["p"] + rand(-100000, 200000);
            $data["gpu"][] = [
                "name" => $brand . " " . $tmpl["base"],
    ...
    // Generate Mobo
    foreach ($mobo_templates as $tmpl) {
        foreach ($brands_mobo as $brand) {
            $price_variation = $tmpl["p"] + rand(-50000, 150000);
            $data["motherboard"][] = [
                "name" => $brand . " " . $tmpl["base"],
                "price" => $price_variation,
                "socket" => $tmpl["s"],
    ...
    // Generate RAM
    foreach ($ram_templates as $tmpl) {
        foreach ($brands_ram as $brand) {
            $price_variation = $tmpl["p"] + rand(-20000, 100000);
            $data["ram"][] = [
                "name" => $brand . " " . $tmpl["base"],
                "price" => $price_variation,
                "type" => $tmpl["type"],
    ...
    // Generate PSU
    foreach ($psu_templates as $tmpl) {
        foreach ($brands_psu as $brand) {
            $data["psu"][] = [
                "name" => $brand . " " . $tmpl["base"],
                "price" => $tmpl["p"] + rand(-50000, 100000),
                "wattage" => $tmpl["w"]
            ];
        }
    }
    ...
    // Generate Storage
    foreach ($storage_templates as $tmpl) {
        foreach ($brands_storage as $brand) {
            $data["storage"][] = [
                "name" => $brand . " " . $tmpl["base"],
                "price" => $tmpl["p"] + rand(-20000, 50000),
                "interface" => $tmpl["if"]
            ];
        }
    }
    ...
    // Generate Case
    foreach ($case_templates as $tmpl) {
        foreach ($brands_case as $brand) {
            $data["casing"][] = [
                "name" => $brand . " " . $tmpl["base"],
                "price" => $tmpl["p"] + rand(-50000, 200000),
...
// Variable utama yang akan diimport oleh index.php
$components = generate_data();

// Uncomment baris di bawah ini kalau mau liat total datanya pas dijalankan
// echo "Total CPU: " . count($components['cpu']) . "<br>";
// echo "Total GPU: " . count($components['gpu']) . "<br>";
// echo "Total Mobo: " . count($components['motherboard']) . "<br>";
?>
